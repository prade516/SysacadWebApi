...
namespace DeskTopSysacad.Formulario.List
{
	public partial class frmalumno : Form
	{
		...
		private void LoadForm()
		{
			...
			Int32 tipo = Convert.ToInt32(EnumeradorPublic.Role.Alumno);
			int page = 1;
		...
		{
			frmpersonasingle frm = new frmpersonasingle(new PersonaDTO(), "A", "Alumno", "Alta Alumno");
			frm.ShowDialog();
			LoadForm();
		}
		...
		private void btnmodificar_Click(object sender, EventArgs e)
		{
			...
			frmpersonasingle frm = new frmpersonasingle(getpersona, "M", "Alumno", "Modificar Alumno");
			frm.ShowDialog();
			LoadForm();
		}
		...
		private void btneliminar_Click(object sender, EventArgs e)
		{
			if (MessageBox.Show("Estas seguro de eliminar este registro ?", "Eliminar registro", MessageBoxButtons.YesNo) == DialogResult.Yes)
			{
				Int32 id = Convert.ToInt32(DGVGrilla[0, DGVGrilla.CurrentRow.Index].Value);
				PersonaDTO getpersona = Myproxy().Get(id, "");
				frmpersonasingle frm = new frmpersonasingle(getpersona, "D", "Alumno", "Eliminar Alumno");
				frm.ShowDialog();
				LoadForm();
			}
		}
